 ...
 * @category Encoder
 */
export function encodeGocadVS(
    dfs: DataFrame[] | DataFrame,
    options: GocadEncodeOptions = undefined,
): string {
    let buffer = ''
...
const getGocadEncodeOptions = (
    options: GocadEncodeOptions,
): GocadEncodeOptions => {
    const r = {
        saveAttributes: true,
        saveTopology: true,
    ...
    r.saveAttributes =
        options.saveAttributes !== undefined ? options.saveAttributes : true
    r.saveTopology =
        options.saveTopology !== undefined ? options.saveTopology : true
    r.saveGeometry =
        options.saveGeometry !== undefined ? options.saveGeometry : true
    r.expandAttributes =
        options.expandAttributes !== undefined
            ? options.expandAttributes
            : false
...
Should be equal to 'positions' count (${positions.count}).\n`
    ...
    if (indices && opts.saveTopology) {
        indices.forEach(
            (item) => (buffer += `${combelName} ${item.join(' ')}\n`),
        )
    }
