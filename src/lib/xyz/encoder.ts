 ...
 * @category Encoder
 */
export function encodeXYZ(
    dfs: DataFrame[] | DataFrame,
    options: XYZEncodeOptions = undefined,
): string {
    let buffer = ''
...
const getXYZEncodeOptions = (o: XYZEncodeOptions): XYZEncodeOptions => {
    ...
    if (o.expandAttributes !== undefined && o.expandAttributes === false) {
        console.warn(
            'WARNING: XYZEncodeOptions.expandAttributes is not active yet for the xyz format',
        )
    }
        ...
        Object.entries(df.series).forEach(([name, serie]: [string, Serie]) => {
            if (name !== 'positions') {
                if (serie.count !== positions.count) {
                    throw new Error(
                        `attribute count mistmatch for '${name}' (got ${serie.count}). Should be equal to 'positions' count (${positions.count})`,
                    )
                }
